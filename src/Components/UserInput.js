...
function UserInput(props){// pass the store state to props
    
    // the useState variable playing is initialised with the initialState object
    const [playing,setPlaying] = useState(initalState);
    
    if(props.word[0]!== undefined){//if there is a word in the store state, i.e new game is clicked
   ...
   function handleChange(event){
        ...
        handleNewSet(input,playing.letter,1,playing.guessList,playing.guessLeft);
    };
    ...
    function handleNewSet(newGuess,newLetter,newShowList,newGuessList,newGuessLeft){
        setPlaying({guess:newGuess,letter:newLetter,showList:newShowList,guessList:newGuessList,guessLeft:newGuessLeft});
    }
    ...
    function userGuess(){
        ...
        if(playing.guess!=="" && send === true){//guess isnt blank and is in word
        // update guess, letter and guessArray 
        handleNewSet("",letter,playing.showList,guessArray,playing.guessLeft);
        }
        else if(playing.guess!== "" && send === false){//guess isnt blank and isn't in word
        ...
        }else{//guess is blank, throw error 
    ...
    function Restart(){
        ...
        handleNewSet(initalState.guess,initalState.letter,initalState.showList,[],initalState.guessLeft);
    ...
    function ToggleRestart(){
        return <Button className="restart-game-button" variant="primary" onClick={Restart}>Restart Game</Button>
        }
    
    // the new game button from the DictionaryFilter component
    let newGame = document.getElementsByClassName("new-game-button").item(Button);
    ...
    function NewWord(){
        handleNewSet(initalState.guess,"",initalState.showList,[],initalState.guessLeft); 
    }
    ...
    function MapLists(){
        ...
        const mapCorrect = correct.map((item) => {
            keysC.push(item)
            return <div className="guess-border"><li className="correct-letter" key={keysC}>{item}</li></div>});

        const mapIncorrect = incorrect.map((item) => {
            keysI.push(item)
            return <div className="guess-border"><li className="incorrect-letter" key={keysI}>{item}</li></div>});
    ...
    return (
        <>
        <Game word={word} guesses={playing.letter} game={playing.showList} remaining ={playing.guessLeft}/>
        <WordDisplay word={word} letter={playing.letter} game={playing.guessLeft} guessList={playing.guessList}/>
        ...
        <div className="add-guess">
            <form>
                ...
                <Button 
                variant="primary" className="guess-btn" type="button" onClick={()=> userGuess()}><Icon.QuestionCircleFill/> Guess</Button>
            </form>            
        </div>

...
function DictionaryFileredWord(props){
    ...
    let Arrayfiltered = toArray.filter(word => (word.length >5 && word.length<11));
    ...
    }else{alert("Error! Cannot set id, word or count!")};
    ...
    function ToggleNewWord(){
        return <Button className="new-game-button" variant="success" onClick={NewWord}>New Game</Button>
    }

...
function Game(props){
    ...
    if(game===0){// reset checker on new or restarted game
        checker=[];
    }
    ...
    function Check(){
        ...
        if(test === true && remaining > 0){
            GameWon();
            }else if(remaining === 0){// user has no more guesses
            GameLost();
        }
    }
    ...
    if(wordArray.includes(guesses) && !(checker.includes(guesses))){
        wordArray.forEach((item)=>{// for each letter in the word 
            if(item===guesses){// if the word letter equals the guess letter
    ...
    useEffect(()=>{
        if(remaining === 0){// if no guesses left 
        setLost(true);// user has lost
        }else if(remaining > 0 && test === true){// user has guesses left & guessed all letters
        setWon(true);// user has won
        }
    },[remaining,test]);// useEffect is called on changes to guesses remaining and the boolean test
    ...
    function GameWon(){       
        ...
        <Modal.Header closeButton>
          <Modal.Title>You've Won! <Icon.TrophyFill/> </Modal.Title>
        </Modal.Header>
        <Modal.Body>
